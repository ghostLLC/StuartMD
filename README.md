...
| 资产 | 说明 |
|------|------|
| **`StuartMD-Setup-2.0.0.exe`** | 主版本（Tauri），推荐安装 |
| `StuartMD-Setup-1.13.1.exe` | 旧 pywebview 稳定包，仅供回退 |

- 默认安装：`%LOCALAPPDATA%\StuartMD`  
- 静默示例：`StuartMD-Setup-2.0.0.exe /S`

## 开发（Tauri 为唯一维护线）

后续只更新 `tauri/` + `web/`；`main.py`（pywebview）冻结在 1.13.1。
